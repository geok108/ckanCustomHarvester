...
class CustomHarvester(SpatialHarvester, SingletonPlugin):
    '''
    ...
    def info(self):
        return {
            'name': 'custom',
            'title': 'Custom Harvester',
            'description': 'A custom harvester for the national open data portal of Cyprus'
            }
    ...
    def gather_stage(self,harvest_job,collection_package_id=None):
        ...
        # Get source URL
        csvurl = harvest_job.source.url
        csvfile = re.sub("http://localhost/", "", csvurl)
        
        #open the CSV file
        with open('/usr/lib/ckan/default/src/ckan/ckan/public/' + csvfile) as f:
           rows = csv.reader(f)
           next(rows)
           i = 0
           
           #write an HTML file which will contain the xml links
           Html_file= open("/usr/lib/ckan/default/src/ckan/ckan/public/waf/waftest.html","w")
           Html_file.write("<html><head></head><body>")
                           
           for row in rows:
               #get specific columns that are needed to include in the xml
               dat_title = row[1]
               dat_format = row[3]
               dat_url = row[13]
               
               #just for WMS datasets
               if dat_format == 'WMS':
                   #get the datasets metadata page url
                   dataseturl = urllib2.urlopen("http://" + dat_url)
                   htmlSource = dataseturl.read()
                   soup = BeautifulSoup(htmlSource)
                   #find the url of the  geospatialdataset
                   wmsurl = soup.find('a', href=re.compile('^http://eservices.dls.moi.gov.cy/')).contents[0]
                   hash_object = hashlib.sha1(dat_url)
                   hex_dig = hash_object.hexdigest()
                  
                   #write an ISO19139 standard xml with the appropriate dataset metadata
                   xmlData = open('/usr/lib/ckan/default/src/ckan/ckan/public/waf/exportxml' + str(i) + '.xml', 'w')
                   xmlData.write("""<?xml version="1.0" encoding="UTF-8"?>
                <gmd:MD_Metadata xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gml="http://www.opengis.net/gml/3.2" xmlns:gmx="http://www.isotc211.org/2005/gmx" xmlns:gts="http://www.isotc211.org/2005/gts" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.isotc211.org/2005/gmd http://wis.wmo.int/2011/schemata/iso19139_2007/schema/gmd/gmd.xsd http://www.isotc211.org/2005/gmx http://wis.wmo.int/2011/schemata/iso19139_2007/schema/gmx/gmx.xsd">
                   <gmd:fileIdentifier>""")
                   xmlData.write("<gco:CharacterString>" + str(hex_dig) + "</gco:CharacterString>")
                   xmlData.write("""</gmd:fileIdentifier>
                   <gmd:language>
                   ...
                   <gmd:dateStamp>
                      <gco:DateTime>2012-10-05T16:04:31Z</gco:DateTime>
                   </gmd:dateStamp>
                   ...
                   <gmd:dataSetURI>
                      <gco:CharacterString>http://repository.usgin.org/uri_gin/usgin/dlio/499</gco:CharacterString>
                   </gmd:dataSetURI>
                   <gmd:identificationInfo>
                      <gmd:MD_DataIdentification>
                         <gmd:citation>
                            <gmd:CI_Citation>
                               <gmd:title>""")
                   xmlData.write("<gco:CharacterString>" + dat_title + "</gco:CharacterString>")
                   xmlData.write("""</gmd:title>
                               <gmd:date>
                                  <gmd:CI_Date>
                                     <gmd:date>
                                        <gco:DateTime>2012-02-29T12:00:00</gco:DateTime>
                                     </gmd:date>
                                     <gmd:dateType>
                               ...
                               <gmd:citedResponsibleParty>
                                  <gmd:CI_ResponsibleParty>
                                     <gmd:individualName>
                                        <gco:CharacterString>Richard, Stephen M.</gco:CharacterString>
                                     </gmd:individualName>
                                     <gmd:organisationName>
                                        <gco:CharacterString>No Organization Name Was Given</gco:CharacterString>
                                     </gmd:organisationName>
                               ...
                               <gmd:citedResponsibleParty>
                                  <gmd:CI_ResponsibleParty>
                                     <gmd:individualName>
                                        <gco:CharacterString>USGIN Specifications Drafting Team</gco:CharacterString>
                                     </gmd:individualName>
                                     <gmd:organisationName>
                                        <gco:CharacterString>Arizona Geological Survey for US Geoscience Information Network (USGIN)</gco:CharacterString>
                                     </gmd:organisationName>
                                     <gmd:role>
                                        <gmd:CI_RoleCode codeList="http://standards.iso.org/ittf/PubliclyAvailableStandards/ISO_19139_Schemas/resources/Codelist/gmxCodelists.xml#CI_RoleCode" codeListValue="originator">originator</gmd:CI_RoleCode>
                                     </gmd:role>
                         ...
                         <gmd:abstract>
                            <gco:CharacterString>This document contains maps.</gco:CharacterString>
                         </gmd:abstract>
                         <gmd:status>
                         ...
                         <gmd:descriptiveKeywords>
                            <gmd:MD_Keywords>
                               <gmd:keyword>
                                  <gco:CharacterString>""" + dat_title + """</gco:CharacterString>
                               </gmd:keyword>
                   ...
                   <gmd:distributionInfo>
                      <gmd:MD_Distribution>
                         ...
                         <gmd:transferOptions>
                            <gmd:MD_DigitalTransferOptions id="http___eservices_dls_moi_gov_cy_arcgis_services_National_Topography_GR_MapServer_WmsServer_">
                               <gmd:onLine>
                                  <gmd:CI_OnlineResource>
                                     <gmd:linkage>""")
                   xmlData.write("<gmd:URL>" + wmsurl + "</gmd:URL>")
                   xmlData.write("""</gmd:linkage>
                                     <gmd:name>
                                        <gco:CharacterString>""" + dat_title + """</gco:CharacterString>
                                     </gmd:name>
                                     <gmd:function>
                   ...
                   Html_file.write("""<a href="http://localhost/waf/exportxml""" + str(i) + """.xml">http://localhost/exportxml""" + str(i) + """.xml</a><br>""")
                   log.debug('XML' + str(i) + " finished!")
                   
        Html_file.write("</body></html>")           
        Html_file.close
           
        #use the HTML with the xml links as the harvest source
        source_url = "http://localhost/waf/waftest.html"
